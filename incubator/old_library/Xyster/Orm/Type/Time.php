...
/**
 * Maps a SQL TIME to a Zend_Date
 *
 * @category  Xyster
 * @package   Xyster_Orm
 * @copyright Copyright (c) 2007-2008 Irrational Logic (http://irrationallogic.net)
 * @license   http://www.opensource.org/licenses/bsd-license.php New BSD License
 */
class Xyster_Orm_Type_Time extends Xyster_Orm_Type_Mutable
{
    /**
     * @var Xyster_Type
     */
    ...
    {
        return new Zend_Date($value, Zend_Date::TIMES);
    }
    ...
    {
        return new Zend_Date($values[0], Zend_Date::TIMES);
    }    
    ...
    public function isEqual( $a, $b )
    {
        if (! $a instanceof Zend_Date ) {
            /* @var $a Zend_Date */
            $a = new Zend_Date($a, Zend_Date::TIMES);
        }
        return $a->equals($b, Zend_Date::TIMES);
    }
    ...
    public function set(Zend_Db_Statement_Interface $stmt, $value, $index, Xyster_Orm_Session_Interface $sess, array $settable = array() )
    {
        if (! $value instanceof Zend_Date ) {
            $value = new Zend_Date($value, Zend_Date::TIMES);
        }
        if ( !count($settable) || $settable[0] ) {
            $stmt->bindValue($index, $value->get(Zend_Date::TIMES));
        }
    }   
}