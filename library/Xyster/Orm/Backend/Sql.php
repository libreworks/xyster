...
class Xyster_Orm_Backend_Sql extends Xyster_Orm_Backend_Abstract
{
	...
	{
	    $keyNames = (array) $this->_mapper->getEntityMeta()->getPrimary();
	...
	public function insert( Xyster_Orm_Entity $entity )
	{
         ...
         */
        $primary = (array) $this->_mapper->getEntityMeta()->getPrimary();
        $pkIdentity = $primary[0];
        if ( count($primary) > 0 ) {  
	        $fields = $this->_mapper->getEntityMeta()->getFields();
        ...
        $sequence = $this->_mapper->getOption('sequence');
         ...
         */
        if ( $this->_mapper->getOption('sequence') && !$data[$pkIdentity]) {
            $data[$pkIdentity] = $this->_getAdapter()->nextSequenceId($this->_mapper->getOption('sequence'));
        }
        ...
        if (!$this->_mapper->getOption('sequence')) {
            /**
